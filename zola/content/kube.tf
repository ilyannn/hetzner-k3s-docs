...
module "kube-hetzner" {
  ...
  # For the server type, the minimum instance supported is cax11 (just a few cents more than cx11) but the cax11 is even cheaper and more powerful (arm64); see https://www.hetzner.com/cloud.
  ...
  k3s_registries = <<-EOT
    mirrors:
      hub.docker.com:
        endpoints:
          - https://registry.cluster.megaver.se/v2/hub.docker.com
          - https://hub.docker.com/v2
        overridePath: true
      registry.k8s.io:
        endpoints:
          - https://registry.cluster.megaver.se/v2/registry.k8s.io
          - https://registry.k8s.io/v2
        overridePath: true
    configs:
      registry.cluster.megaver.se:
        auth:
          username: robot$cluster
          password: REDACTED
  EOT
  ...
  firewall_kube_api_source = REDACTED
